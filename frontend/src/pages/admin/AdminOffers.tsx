// Admin — offers monitor across ALL clients, with reach/click performance.
...
import { viewsMaxApi, type AdminOfferRow } from "@/lib/api-service";
...
const COLUMNS: AdminColumn<AdminOfferRow>[] = [
  { key: "name", head: "Offer", sort: (r) => (r.name || r.url || "").toLowerCase(), render: (r) => (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontWeight: 700, color: "var(--ink-on-paper-1)" }}>{r.name || "—"}</div>
      <div style={subText}>{r.url}</div>
    </div>
  ) },
  { key: "user", head: "User", sort: (r) => (r.user?.email || "").toLowerCase(), render: (r) => (
    <div style={{ minWidth: 0 }}>
      <div>{r.user?.name || "—"}</div>
      <div style={subText}>{r.user?.email || `#${r.id}`}</div>
    </div>
  ) },
  { key: "links_count", head: "Links", align: "right", sort: (r) => r.links_count, render: (r) => r.links_count },
...
export default function AdminOffers() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await viewsMaxApi.getAdminOffers(dq || undefined);
  ...
  return (
    <PostShell max={1560}>
      <SectionHead eyebrow="Admin" title="Offers" />
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search offer, url, or user…" style={adminSearchStyle} />
      </div>
      {loading ? (
        <AnalyticsLoading />
      ) : rows.length === 0 ? (
        <div style={{ background: "var(--paper-0)", border: "1px solid var(--line-1)", borderRadius: 18, padding: 48, textAlign: "center", color: "var(--ink-on-paper-3)", fontFamily: "var(--font-body)", fontSize: 14 }}>No offers match.</div>
      ) : (
        <AdminMetricTable rows={rows} columns={COLUMNS} minWidth={1040} initialSort="revenue" />
      )}
    </PostShell>
  );
}

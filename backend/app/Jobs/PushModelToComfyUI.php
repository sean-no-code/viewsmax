...
class PushModelToComfyUI implements ShouldQueue
{
    ...
    {
        try {
            Log::info('=== PUSH MODEL TO COMFYUI JOB STARTED ===', [
            ...
            Log::info('AI Model found for ComfyUI push', [
            ...
            // Check if we have the Replicate prediction ID
            if (!$aiModel->replicates_prediction_id) {
                Log::error('Missing Replicate prediction ID', [
                    'ai_model_id' => $aiModel->id
                ]);
                throw new \Exception('Missing Replicate prediction ID');
            }
            ...
            $loraLocalPath = $filesToUpload['lora.safetensors'] ?? null;
            
            if (!$loraLocalPath || !file_exists($loraLocalPath)) {
                Log::error('LoRA file not found in processed files', [
                    'ai_model_id' => $aiModel->id,
                    'files_to_upload' => array_keys($filesToUpload)
                ]);
                throw new \Exception('LoRA file not found after extraction');
            }

            Log::info('LoRA file ready for upload', [
                'ai_model_id' => $aiModel->id,
                'local_path' => $loraLocalPath,
                'file_size_mb' => round(filesize($loraLocalPath) / 1024 / 1024, 2)
            ]);

            // Upload to ComfyUI server via SCP
            $uploadSuccess = $this->uploadToComfyUI($aiModel, $loraLocalPath);
            
            if (!$uploadSuccess) {
                Log::error('Failed to upload LoRA to ComfyUI server', [
                    'ai_model_id' => $aiModel->id
                ]);
                throw new \Exception('Failed to upload LoRA to ComfyUI server');
            }
            ...
            Log::info('=== PUSH MODEL TO COMFYUI JOB COMPLETED ===', [
                'ai_model_id' => $aiModel->id
            ]);

        } catch (\Exception $e) {
            Log::error('PushModelToComfyUI job failed', [
                'ai_model_id' => $this->aiModelId,
    ...
    /**
     * Upload LoRA file to ComfyUI server via SCP
     *
     * @param AiModel $aiModel
     * @param string $localPath Path to local lora.safetensors file
     * @return bool
     */
    private function uploadToComfyUI(AiModel $aiModel, string $localPath): bool
    {
        try {
            // Get ComfyUI SSH configuration
            ...
            Log::info('Preparing SCP upload to ComfyUI', [
                'ai_model_id' => $aiModel->id,
                'local_path' => $localPath,
                'local_file_size_mb' => round(filesize($localPath) / 1024 / 1024, 2),
            ...
            if (!file_exists($sshKeyPath)) {
                Log::error('SSH key file not found', [
                    'ssh_key_path' => $sshKeyPath
                ]);
                throw new \Exception("SSH key file not found: {$sshKeyPath}");
            }

            // Build SCP command
            // scp -i ~/.ssh/key.pem -o StrictHostKeyChecking=no localfile user@host:/remote/path
            $scpCommand = sprintf(
                'scp -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s %s@%s:%s 2>&1',
            ...
            Log::info('Executing SCP command', [
                'ai_model_id' => $aiModel->id,
                'command' => $scpCommand
            ]);
            ...
            Log::info('SCP upload successful', [
                'ai_model_id' => $aiModel->id,
                'target_filename' => $targetFilename,
                'remote_path' => $remoteFilePath
            ]);

            // Verify the upload by checking if the LoRA exists on ComfyUI server
            try {
                // Give server a moment to recognize the new file
                sleep(2);
                
                $loraExists = $comfyUIService->loraExists($targetFilename);
                
                if ($loraExists) {
                    Log::info('Verified LoRA exists on ComfyUI server', [
                        'ai_model_id' => $aiModel->id,
                        'lora_filename' => $targetFilename
                    ]);
                } else {
                    Log::warning('LoRA uploaded but not found in ComfyUI server list (may need server restart)', [
                        'ai_model_id' => $aiModel->id,
                        'lora_filename' => $targetFilename
                    ]);
                }
            } catch (\Exception $e) {
                Log::warning('Could not verify LoRA on ComfyUI server', [
                    'ai_model_id' => $aiModel->id,
                    'error' => $e->getMessage()
                ]);
            }
            ...
            Log::error('Exception during ComfyUI upload', [
                'ai_model_id' => $aiModel->id,
                'error' => $e->getMessage(),
    ...
    private function downloadFile(string $url): ?string
    {
        try {
            ...
            // Create temp file
            $tempFile = tempnam(sys_get_temp_dir(), 'comfyui_model_');
    ...
    private function extractTarFile(string $tarPath): array
    {
        ...
        try {
            ...
            $extractDir = sys_get_temp_dir() . '/comfyui_extract_' . uniqid();
    ...
    private function cleanupTempFiles(): void
    {
        try {
            ...
            $modelFiles = glob($tempDir . '/comfyui_model_*');
            ...
            $extractDirs = glob($tempDir . '/comfyui_extract_*');
    ...
    {
        Log::error('PushModelToComfyUI job permanently failed', [
            'ai_model_id' => $this->aiModelId,
            'error' => $exception->getMessage(),
            'attempts' => $this->attempts()
        ]);

        // Update the AI model status to indicate the push failed
        try {
            $aiModel = AiModel::find($this->aiModelId);
            if ($aiModel) {
                $aiModel->update([
                    'error_message' => 'Failed to push model to ComfyUI: ' . $exception->getMessage()
                ]);
            }
        } catch (\Exception $e) {
